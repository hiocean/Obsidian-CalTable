import { Editor, MarkdownRenderer, MarkdownView, Plugin } from "obsidian";
...
export default class CalTable extends Plugin {
    ...
    async onload() {
        ...
        this.registerMarkdownCodeBlockProcessor(
            ...
            (source, el) => {
                ...
                const dots = ".".repeat(this.settings.repeatTimes);
                const unit: string =
                    this.getVariables()?.currency || this.settings.suffix;
        ...
        this.addCommand({
            id: "upper-lower-case",
            name: "Rotate Upper and Lower the selected Text",
            editorCallback: (editor: Editor) => {
                ...
                let rotatedText = this.rotateText(selectedText);
                editor.replaceSelection(rotatedText);
            },
        });
        this.addSettingTab(new CalTableSettingTab(this.app, this));
    }

    private rotateText(text: string) {
        let rotatedText = "";
        if (text === text.toUpperCase()) {
            rotatedText = text.toLowerCase();
        } else if (text === text.toLowerCase()) {
            rotatedText = text.charAt(0).toUpperCase() + text.slice(1);
        } else if (text === text.charAt(0).toUpperCase() + text.slice(1)) {
            rotatedText = text.toUpperCase();
        }
        return rotatedText;
    }
    ...
    private generateTable(
        ...
        lines.forEach((inputText) => {
            const calulatedValue = this.evalRow({
                textOfLine: inputText,
            ...
            MarkdownRenderer.renderMarkdown(inputText+dots, c1Input);
            c1Input.addEventListener("keydown", (event) => {
                if (event.key === "Enter") {
                    // 判断是否按下回车键
                    event.preventDefault(); // 阻止默认操作
                    const outputText = c1Input.innerText
                        .replace(dots, "")
                        .trim(); // 获取文本内容，并去除前后空格
            ...
            const c2 = rowEL.createEl("td", {
                text: calulatedValue.toFixed(0),
                attr: { style: "text-align: right; vertical-align: bottom;" },
            });
            const c3 = rowEL.createEl("td", {
                text: unit,
                attr: { style: "text-align: right; vertical-align: bottom;" },
            });
            if (
                inputText.match(
                    new RegExp(this.settings.specialCellKeyword, "gi")
                )
            ) {
    ...
    private evalRow({
        ...
        if (match) {
            const valueForCurrRow = this.evalStringWithVars(
                match[1],
                this.getVariables()
            );
            const sum =
                length >= 1
                    ? accumulatedValues[length - 1][1] + valueForCurrRow
                    : valueForCurrRow;
    ...
    private evalStringWithVars(s: string, variables: { [x: string]: string }) {
        ...
        Object.keys(variables).forEach((key) => {
            input = input.replace(
                key,
                variables[key] + this.settings.defaultOperator
            );
        });
        input = input
            .replace(/[;]/g, "+")
            .split(" ") // 将字符串按空格拆分成数组元素
            ...
            .join("")
            .replace(/%/g, "* 0.01"); //合并并替换%符号，这样就不用mathjs了
        // console.log(arr);
        return eval(input);
    }
    ...
    private getVariables() {
        ...
        if (tf) {
            variables = {
                ...variables,
                ...this.app.metadataCache.getFileCache(tf)?.frontmatter,
            };
        }
        return variables;
    }
